...
`src/schema/contract.ts` holds the zod schemas for `RunResult`, `FileResult`, the events and `CompareResult`, and `src/pipeline/types.ts` and `src/compare/types.ts` derive the public types from them with `z.infer`. zod takes about 60 ms to import, so no runtime module imports `contract.ts`; only the build, the tests and the UI server do. `scripts/build-schema.mjs` writes `dist/schema/*.schema.json` with `z.toJSONSchema(schema, { io: "input" })`, which leaves objects open so consumers accept added fields. Tests check a result with `expect(schema.parse(result)).toEqual(result)`, since parsing strips any field the schema lacks. Enums the contract shares with other modules are `as const` tuples there, such as `INSPECT_FORMATS`, `METRICS_VERDICTS`, `STRIP_REMOVED_KINDS` and `ENCODE_METHODS`.

## Comparisons and markup

- `compareFiles` (`src/compare/`) is what `wio compare` runs. It reads and inspects each image with `readInput` and `inspect` (refusing SVG), checks their dimensions, checks the diff map's path against both images with `planWrites`, scores them with `decodeForScoring` and `score`, and writes `createDiffMap`'s PNG through `writeOutputs`. As in `optimiseFile`, an `OptimiserError` becomes the result's `error`, its message prefixed with the image's path. The result's schema is `compareResultSchema`.
- `generatePictureMarkup` (`src/markup/`) is pure. The last output is the `<img>`, which every browser can show, and the others are `<source>`s. It needs `FileResult.width` and `height`, which `optimiseFile` reports from `inspect`.

## CLI

`src/bin/` builds a `CliIo` from the process: the streams, colour from `stdout.hasColors(process.env)` (off for `NO_COLOR`), progress when stderr is a terminal, and an `AbortSignal` for Ctrl+C and SIGTERM. It passes that to `runCli` in `src/cli/`, which returns the exit code, so the tests run the whole CLI in-process. `docs/cli.md` is the user-facing account.

- **Program:** commander. `defineOptimise` puts the optimise arguments, flags and action on both the program itself, so `wio <inputs>` needs no command name, and its `optimise` (alias `optimize`) subcommand. `enablePositionalOptions` stops the program taking `compare`'s `--json` as its own. `exitOverride` turns every usage error into a `CommanderError`, which `runCli` maps to exit 2, and help and `--version` to 0. An action reports its own usage errors, such as `--markup` outside `suite` or `E_NO_INPUTS`, through `command.error`, so they print the same way.
- **Inputs** (`expandInputs.ts`): a path that exists is taken as it is. A folder is scanned by extension with tinyglobby (subfolders with `--recursive`), leaving out an `--out-dir` inside it. An argument that doesn't exist but has glob characters is globbed, with `\` turned into `/` on Windows and matching case-insensitively on Windows and macOS. A pattern matching no image is passed on as a path, so it fails with `E_READ`. Scanned files get their subfolder, mirrored under `--out-dir`, as their own `outDir`. A scan leaves out a file in the mode's WebP or AVIF format named as another input's output beside it, taking it for an earlier run's output. Inputs are de-duplicated and sorted by `comparablePath`.
- **Output:** results go to stdout (`renderRun`'s table, `--json` or `--ndjson`), and everything else to stderr. `withHint` appends the flag that fixes `E_OUTPUT_IS_INPUT` or `W_OUTPUT_EXISTS` to its message, in the JSON too, because the library's messages don't name CLI flags. `--markup` adds `generatePictureMarkup(file, { root: outDir })` to each file.
- **Signals:** the first Ctrl+C or SIGTERM aborts the batch, which rejects once the files in progress have cleaned up, and `src/bin/` then exits with 128 plus the signal's number. A second signal calls `process.exit` at once.
- **Tests:** `tests/cli/` runs `runCli` in-process with captured streams. `tests/bin/wio.test.ts` spawns the built `dist/bin/index.js` (it is skipped without `dist/`), runs `SKILL.md`'s commands as written, and on POSIX checks that Ctrl+C exits 130 leaving no temp files.
- **`SKILL.md`**, at the root, is the agent guide that ships in `dist/`. Keep its commands runnable, since the end-to-end test runs them.
